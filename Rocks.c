...
void MoveAlienShips() {
   ...
   // movement along x axis
      ...
      // a check in case the asteroid now has active equals 0
         if (palien->active > 0) {
         //w l2("Adding pointer for Aliensd", sltoa(i));
...
int IsEmptySpace(int x, int y) {
   ...
   int celly = y/CELLSIZE;
   int cellx = x/CELLSIZE;
   if (cells[cellx][celly].numobjects == 0 && cells[cellx + 1][celly].numobjects == 0 && cells[cellx][celly + 1].numobjects == 0 && cells[cellx + 1][celly + 1].numobjects == 0) return 1;
   return 0;
}
